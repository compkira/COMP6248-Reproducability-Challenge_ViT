...
# #     p_c.data[:] = p_o.data
# valloader = DataLoader(valset, 1)

# with torch.no_grad():

#     # reshape the image and change it to float tensor
#     img_width, img_height = mc.image_size[1], mc.image_size[2]
#     patch_width, patch_height = mc.patch_size[0], mc.patch_size[1]
#     patch_num_width = img_width // patch_width
#     patch_num_height = img_height // patch_height
#     head_num = mc.num_heads
#     print(patch_num_width, patch_num_height)

#     tc.parse(batch_size=1, num_workers=1)
#     inp, _ = next(iter(valloader))
#     print(inp.shape)
#     # inp = torch.from_numpy(np.array(Image.open("img1.jpg").resize([img_width, img_height]))) * 1.
#     # # reshape image to B C H W
#     # # h: image width = 384, w: image height = 384, c: image channel = 3
...
#     # p = pos_embedding[0, 0, :]
#     pos_embedding = pos_embedding[0, 1:, :] # 576, 768
#     # # pos_embedding = rearrange(pos_embedding, "(p1 p2) d -> p1 p2 d", p1=24)
#     # print(pos_embedding.shape)

#     # # cosine_similarity = torch.zeros(pos_embedding.shape[0], pos_embedding.shape[1], pos_embedding.shape[0], pos_embedding.shape[1])
#     # # for i in range(pos_embedding.shape[0]):
#     # #     for j in range(pos_embedding.shape[1]):
            
#     norm = torch.norm(pos_embedding, 2, dim=1, keepdim=True)
#     cosine_similarity = pos_embedding @ pos_embedding.T / (norm @ norm.T)
#     cosine_similarity = rearrange(cosine_similarity, "(p1 p2) (h w) -> p1 p2 h w", p1=patch_num_width, h=patch_num_height)
#     print(cosine_similarity.shape)
...
#     for i in range(cosine_similarity.shape[0]):
#         for j in range(cosine_similarity.shape[1]):
#             # ax = plt.subplot(cosine_similarity.shape[0], cosine_similarity.shape[1], (i*cosine_similarity.shape[1]+1)+j)
#             images.append(ax[i, j].imshow(cosine_similarity[i, j]))
#             ax[i, j].label_outer()
...
#     fig.colorbar(mappable=images[0], ax=ax)
#     fig.suptitle("Positional Encoding")
#     fig.savefig("src/main/resources/images/PosEmbeddingCIFAR100.jpg", dpi=300)
#     plt.show()